...
export default function useUser() {
  ...
  const updateEmail = useCallback(
    async (data: ChangeEmailData): Promise<HookResult> => {
      try {
        ...
        if (error instanceof AxiosError) {
          result.current = {
            response: null,
            error: ErrorsHandling("user_update_email", error),
          };
        } else {
          result.current = {
            response: null,
  ...
  const updatePassword = useCallback(
    async (data: ChangePasswordData): Promise<HookResult> => {
      try {
        ...
        if (error instanceof AxiosError) {
          result.current = {
            response: null,
            error: ErrorsHandling("user_update_password", error),
          };
        } else {
          result.current = {
            response: null,
